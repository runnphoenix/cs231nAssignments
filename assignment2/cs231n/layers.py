...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = x, u, var, x_norm, gamma, eps
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  x, u, var, x_norm, gamma, eps = cache
  dx_h = dout * gamma
  dvar = np.sum(dx_h * (x-u) * -0.5 *np.power(var+eps, -1.5), axis=0)
  du = np.sum(-dx_h / np.sqrt(var+eps), axis=0)
  dx = dx_h/np.sqrt(var+eps) + 2*(x-u)/float(x.shape[0])*dvar + du/float(x.shape[0])
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in range(N):
    for c in range(C):
      for j in range(Ho):
        for i in range(Wo):
          index = np.argmax(x[n,c,j*stride:j*stride+ph,i*stride:i*stride+pw])
          index_j = index / pw
          index_i = index - index_j * pw
          dx[n,c,j*stride+index_j,i*stride+index_i] = dout[n,c,j,i]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  #############################################################################
  N,C,H,W = x.shape
  transed_x = x.transpose(0,2,3,1).reshape(-1,x.shape[1])
  out, cache = batchnorm_forward(transed_x, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
  ...
  #############################################################################
  N,C,H,W = dout.shape
  transed_dout = dout.transpose(0,2,3,1).reshape(-1,C)
  dx,dgamma,dbeta = batchnorm_backward(transed_dout, cache)
